package org.crossflow.runtime.serialization;
...
import org.crossflow.runtime.Bid;
import org.crossflow.runtime.BidOffer;
...
public class JsonSerializer extends AbstractSerializer {
	...
	public JsonSerializer(boolean pretty, boolean isStrict) {
		...
		GsonBuilder builder = new GsonBuilder()
				.registerTypeAdapterFactory(new ClassTypeAnnotatingTypeAdapterFactory())
	...
	public <O> O deserialize(String input) {
		...
		if (jsonObject.has("job")) {
			return deserializeObjectWithNestedJob(jsonObject);
		} else {
			return doDeserialize(jsonObject);
		}
	}

	private <O> O deserializeObjectWithNestedJob(JsonObject jsonObject) {
		O deserialized = doDeserialize(jsonObject);
		if (deserialized instanceof Bid) {
			Bid bid = (Bid) deserialized;
			return deserializeBid(jsonObject, bid);
		}
		if (deserialized instanceof BidOffer) {
			BidOffer bid = (BidOffer) deserialized;
			return deserializeBidOffer(jsonObject, bid);
		}
		return deserialized;
	}

	private <O> O deserializeBidOffer(JsonObject jsonObject, BidOffer bid) {
		Object job = doDeserialize(jsonObject.getAsJsonObject("job"));
		Job job1 = (Job) job;
		bid.setJob(job1);
		return (O) bid;
	}

	private <O> O deserializeBid(JsonObject jsonObject, Bid bid) {
		Object job = doDeserialize(jsonObject.getAsJsonObject("job"));
		Job job1 = (Job) job;
		bid.setJob(job1);
		return (O) bid;
