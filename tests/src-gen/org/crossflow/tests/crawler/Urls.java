...
public class Urls extends JobStream<Url> {
	...
	public Urls(Workflow<CrawlerWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("UrlCollector", (ActiveMQDestination) session.createQueue("UrlsPre.UrlCollector." + workflow.getInstanceId()));
			destination.put("UrlCollector", (ActiveMQDestination) session.createQueue("UrlsDestination.UrlCollector." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("UrlsPost.UrlCollector." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("UrlCollector", postQ);			
					...
					try {
						...
						if (workflow.getCache() != null && workflow.getCache().hasCachedOutputs(job)) {
							...
							for (Job output : cachedOutputs) {
								if (output.getDestination().equals("UrlsToAnalyse")) {
									workflow.cancelTermination();
									((CrawlerWorkflow) workflow).getUrlsToAnalyse().send((Url) output, consumerId);
								}
