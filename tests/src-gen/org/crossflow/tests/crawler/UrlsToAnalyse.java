...
@Generated(value = "org.crossflow.java.Steam2Class", date = "2022-01-05T17:43:31.905448Z")
public class UrlsToAnalyse extends JobStream<Url> {
	...
	protected Task cacheManagerTask = new Task() {
		...
		@Override
		public String getId() {
			return "CacheManager:UrlsToAnalyse";
		}
	...
	public UrlsToAnalyse(Workflow<CrawlerWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("UrlAnalyser", (ActiveMQDestination) session.createQueue("UrlsToAnalysePre.UrlAnalyser." + workflow.getInstanceId()));
			destination.put("UrlAnalyser", (ActiveMQDestination) session.createQueue("UrlsToAnalyseDestination.UrlAnalyser." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("UrlsToAnalysePost.UrlAnalyser." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("UrlAnalyser", postQ);			
					...
					try {
						...
						if (workflow.getCache() != null && workflow.getCache().hasCachedOutputs(job)) {
							...
							for (Job output : cachedOutputs) {
								if (output.getDestination().equals("Urls")) {
									workflow.cancelTermination();
									((CrawlerWorkflow) workflow).getUrls().send((Url) output, consumerId);
								}
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(url != null)
						...
						System.err.println("INFO: UrlsToAnalyse ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
