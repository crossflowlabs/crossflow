...
public class Out2 extends JobStream<Number2> {
	...
	public Out2(Workflow<MultiflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("MultiSink", (ActiveMQDestination) session.createQueue("Out2Pre.MultiSink." + workflow.getInstanceId()));
			destination.put("MultiSink", (ActiveMQDestination) session.createQueue("Out2Destination.MultiSink." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("Out2Post.MultiSink." + workflow.getInstanceId()
	...
	public void addConsumer(Out2Consumer consumer, String consumerId) throws Exception {
		...
		if (consumer != null) {		
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(number2 != null)
						consumer.consumeOut2WithNotifications(number2);
					else
						System.err.println("INFO: Out2 ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
