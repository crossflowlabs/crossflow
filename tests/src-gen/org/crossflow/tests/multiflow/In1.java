...
@Generated(value = "org.crossflow.java.Steam2Class", date = "2021-10-26T15:08:26.086642Z")
public class In1 extends JobStream<Number2> {
	...
	public In1(Workflow<MultiflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("MultiTask", (ActiveMQDestination) session.createQueue("In1Pre.MultiTask." + workflow.getInstanceId()));
			destination.put("MultiTask", (ActiveMQDestination) session.createQueue("In1Destination.MultiTask." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("In1Post.MultiTask." + workflow.getInstanceId()
	...
	public void addConsumer(In1Consumer consumer, String consumerId) throws Exception {
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(messageText != null && messageText.length() > 0)
						number2 = workflow.getSerializer().deserialize(messageText);
					if(number2 != null)
						consumer.consumeIn1WithNotifications(number2);
					else
						System.err.println("INFO: In1 ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
