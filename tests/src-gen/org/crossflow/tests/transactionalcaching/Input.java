...
@Generated(value = "org.crossflow.java.Steam2Class", date = "2021-10-26T15:08:27.566934Z")
public class Input extends JobStream<Element> {
	...
	public Input(Workflow<MinimalWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		...
		ActiveMQDestination postQ;
			pre.put("ClonerTask", (ActiveMQDestination) session.createQueue("InputPre.ClonerTask." + workflow.getInstanceId()));
			destination.put("ClonerTask", (ActiveMQDestination) session.createQueue("InputDestination.ClonerTask." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("InputPost.ClonerTask." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("ClonerTask", postQ);			
						...
						if (workflow.getCache() != null && workflow.getCache().hasCachedOutputs(job)) {
							...
							for (Job output : cachedOutputs) {
								if (output.getDestination().equals("Output")) {
									workflow.cancelTermination();
									((MinimalWorkflow) workflow).getOutput().send((Element) output, consumerId);
								}
		...
		if (consumer != null) {		
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(element != null)
						consumer.consumeInputWithNotifications(element);
					else
