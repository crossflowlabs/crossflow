...
public class WordFrequencies extends JobStream<WordFrequency> {
	...
	protected Task cacheManagerTask = new Task() {
		...
		@Override
		public String getId() {
			return "CacheManager:WordFrequencies";
		}
	...
	public WordFrequencies(Workflow<WordCountWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("WordCountSink", (ActiveMQDestination) session.createQueue("WordFrequenciesPre.WordCountSink." + workflow.getInstanceId()));
			destination.put("WordCountSink", (ActiveMQDestination) session.createQueue("WordFrequenciesDestination.WordCountSink." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("WordFrequenciesPost.WordCountSink." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("WordCountSink", postQ);			
	...
	public void addConsumer(WordFrequenciesConsumer consumer, String consumerId) throws Exception {
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(messageText != null && messageText.length() > 0)
						wordFrequency = workflow.getSerializer().deserialize(messageText);
					if(wordFrequency != null)
						consumer.consumeWordFrequenciesWithNotifications(wordFrequency);
					else
						System.err.println("INFO: WordFrequencies ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
