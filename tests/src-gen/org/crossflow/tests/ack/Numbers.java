...
@Generated(value = "org.crossflow.java.Steam2Class", date = "2021-10-26T15:08:21.360808Z")
public class Numbers extends JobStream<IntElement> {
	...
	public Numbers(Workflow<AckWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("ProcessingTask", (ActiveMQDestination) session.createQueue("NumbersPre.ProcessingTask." + workflow.getInstanceId()));
			destination.put("ProcessingTask", (ActiveMQDestination) session.createQueue("NumbersDestination.ProcessingTask." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("NumbersPost.ProcessingTask." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("ProcessingTask", postQ);			
						...
						if (workflow.getCache() != null && workflow.getCache().hasCachedOutputs(job)) {
							...
							for (Job output : cachedOutputs) {
								if (output.getDestination().equals("Results")) {
									workflow.cancelTermination();
									((AckWorkflow) workflow).getResults().send((StringElement) output, consumerId);
								}
	...
	public void addConsumer(NumbersConsumer consumer, String consumerId) throws Exception {
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(messageText != null && messageText.length() > 0)
						intElement = workflow.getSerializer().deserialize(messageText);
					if(intElement != null)
						consumer.consumeNumbersWithNotifications(intElement);
					else
						System.err.println("INFO: Numbers ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
