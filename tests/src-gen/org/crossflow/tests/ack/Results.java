...
@Generated(value = "org.crossflow.java.Steam2Class", date = "2022-01-05T17:43:27.231342Z")
public class Results extends JobStream<StringElement> {
	...
	public Results(Workflow<AckWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("Sink", (ActiveMQDestination) session.createQueue("ResultsPre.Sink." + workflow.getInstanceId()));
			destination.put("Sink", (ActiveMQDestination) session.createQueue("ResultsDestination.Sink." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("ResultsPost.Sink." + workflow.getInstanceId()
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(messageText != null && messageText.length() > 0)
						stringElement = workflow.getSerializer().deserialize(messageText);
					if(stringElement != null)
						consumer.consumeResultsWithNotifications(stringElement);
					else
