...
public class PostConfigTopic extends JobStream<Post> {
	...
	protected Task cacheManagerTask = new Task() {
		...
		@Override
		public String getId() {
			return "CacheManager:PostConfigTopic";
		}
	...
	public PostConfigTopic(Workflow<AdditionWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("Adder", (ActiveMQDestination) session.createQueue("PostConfigTopicPre.Adder." + workflow.getInstanceId()));
			destination.put("Adder", (ActiveMQDestination) session.createQueue("PostConfigTopicDestination.Adder." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createTopic("PostConfigTopicPost.Adder." + workflow.getInstanceId()
	...
	public void addConsumer(PostConfigTopicConsumer consumer, String consumerId) throws Exception {
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(messageText != null && messageText.length() > 0)
						post = workflow.getSerializer().deserialize(messageText);
					if(post != null)
						consumer.consumePostConfigTopicWithNotifications(post);
					else
						System.err.println("INFO: PostConfigTopic ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
