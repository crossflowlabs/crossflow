...
@Generated(value = "org.crossflow.java.Workflow2Class", date = "2022-01-05T17:43:31.251449Z")
public class AdditionWorkflow extends Workflow<AdditionWorkflowTasks> {
	...
	protected OperationConfigSource operationConfigSource;
	protected PostConfigSource postConfigSource;
	...
	public AdditionWorkflow(Mode mode, int parallelization) {
		...
		if (isMaster()) {
			...
			operationConfigSource = new OperationConfigSource();
			operationConfigSource.setWorkflow(this);
			tasks.add(operationConfigSource);
			postConfigSource = new PostConfigSource();
			postConfigSource.setWorkflow(this);
			tasks.add(postConfigSource);
		}
	...
	public void run(long delay) throws Exception {
		...
		try {
			...
			operationConfigTopic = new OperationConfigTopic(AdditionWorkflow.this, enablePrefetch);
			activeStreams.add(operationConfigTopic);
			postConfigTopic = new PostConfigTopic(AdditionWorkflow.this, enablePrefetch);
			activeStreams.add(postConfigTopic);
		
			if (isMaster()) {
					numberPairSource.setAdditions(additions);
					additionResults.addConsumer(additionResultsSink, "AdditionResultsSink");			
					operationConfigSource.setOperationConfigTopic(operationConfigTopic);
					postConfigSource.setPostConfigTopic(postConfigTopic);
			}
			...
			if (isWorker()) {
				if (!tasksToExclude.contains(AdditionWorkflowTasks.ADDER)) {
						for(int i = 0; i <adders.size(); i++){
							...
							operationConfigTopic.addConsumer(task, "Adder");			
							postConfigTopic.addConsumer(task, "Adder");			
	...
	public void sendConfigurations(){
		...
		new Thread(() -> {
					try {
						setTaskInProgess(operationConfigSource);
						operationConfigSource.produce();
						setTaskWaiting(operationConfigSource);
					} catch (Exception ex) {
						reportInternalException(ex);
						terminate();
					}
		...
		new Thread(() -> {
					try {
						setTaskInProgess(postConfigSource);
						postConfigSource.produce();
						setTaskWaiting(postConfigSource);
					} catch (Exception ex) {
						reportInternalException(ex);
						terminate();
					}
	...
	public OperationConfigTopic getOperationConfigTopic() {
		return operationConfigTopic;
	}
	public PostConfigTopic getPostConfigTopic() {
	...
	public OperationConfigSource getOperationConfigSource() {
		return operationConfigSource;
	}
	public PostConfigSource getPostConfigSource() {
		return postConfigSource;
	}
	...
	protected void registerCustomSerializationTypes(Serializer serializer) {
		...
		serializer.registerType(Operation.class);
		serializer.registerType(Post.class);
	}
