...
public class Results extends JobStream<Number> {
	...
	public Results(Workflow<ExceptionsWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("ResultsSink", (ActiveMQDestination) session.createQueue("ResultsPre.ResultsSink." + workflow.getInstanceId()));
			destination.put("ResultsSink", (ActiveMQDestination) session.createQueue("ResultsDestination.ResultsSink." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("ResultsPost.ResultsSink." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("ResultsSink", postQ);			
	...
	public void addConsumer(ResultsConsumer consumer, String consumerId) throws Exception {
		...
		if (consumer != null) {		
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(number != null)
						consumer.consumeResultsWithNotifications(number);
					else
						System.err.println("INFO: Results ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
