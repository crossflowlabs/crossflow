...
@Generated(value = "org.crossflow.java.Steam2Class", date = "2022-01-05T17:43:32.243142Z")
public class Numbers extends JobStream<Number> {
	...
	public Numbers(Workflow<ExceptionsWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("NumberCopier", (ActiveMQDestination) session.createQueue("NumbersPre.NumberCopier." + workflow.getInstanceId()));
			destination.put("NumberCopier", (ActiveMQDestination) session.createQueue("NumbersDestination.NumberCopier." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("NumbersPost.NumberCopier." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("NumberCopier", postQ);			
						...
						if (workflow.getCache() != null && workflow.getCache().hasCachedOutputs(job)) {
							...
							for (Job output : cachedOutputs) {
								if (output.getDestination().equals("Results")) {
									workflow.cancelTermination();
									((ExceptionsWorkflow) workflow).getResults().send((Number) output, consumerId);
								}
		...
		if (consumer != null) {		
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(number != null)
						consumer.consumeNumbersWithNotifications(number);
					else
