...
@Generated(value = "org.crossflow.java.Steam2Class", date = "2021-10-26T15:08:25.592432Z")
public class MatrixConfigurations extends JobStream<MatrixConfiguration> {
	...
	protected Task cacheManagerTask = new Task() {
		...
		@Override
		public String getId() {
			return "CacheManager:MatrixConfigurations";
		}
	...
	public MatrixConfigurations(Workflow<MatrixWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("MatrixConstructor", (ActiveMQDestination) session.createQueue("MatrixConfigurationsPre.MatrixConstructor." + workflow.getInstanceId()));
			destination.put("MatrixConstructor", (ActiveMQDestination) session.createQueue("MatrixConfigurationsDestination.MatrixConstructor." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("MatrixConfigurationsPost.MatrixConstructor." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("MatrixConstructor", postQ);			
					...
					try {
						...
						if (workflow.getCache() != null && workflow.getCache().hasCachedOutputs(job)) {
							...
							for (Job output : cachedOutputs) {
								if (output.getDestination().equals("Matrices")) {
									workflow.cancelTermination();
									((MatrixWorkflow) workflow).getMatrices().send((Matrix) output, consumerId);
								}
	...
	public void addConsumer(MatrixConfigurationsConsumer consumer, String consumerId) throws Exception {
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(messageText != null && messageText.length() > 0)
						matrixConfiguration = workflow.getSerializer().deserialize(messageText);
					if(matrixConfiguration != null)
						consumer.consumeMatrixConfigurationsWithNotifications(matrixConfiguration);
					else
						System.err.println("INFO: MatrixConfigurations ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
