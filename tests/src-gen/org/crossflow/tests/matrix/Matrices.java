...
public class Matrices extends JobStream<Matrix> {
	...
	public Matrices(Workflow<MatrixWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("MatrixSink", (ActiveMQDestination) session.createQueue("MatricesPre.MatrixSink." + workflow.getInstanceId()));
			destination.put("MatrixSink", (ActiveMQDestination) session.createQueue("MatricesDestination.MatrixSink." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("MatricesPost.MatrixSink." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("MatrixSink", postQ);			
	...
	public void addConsumer(MatricesConsumer consumer, String consumerId) throws Exception {
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(messageText != null && messageText.length() > 0)
						matrix = workflow.getSerializer().deserialize(messageText);
					if(matrix != null)
						consumer.consumeMatricesWithNotifications(matrix);
					else
						System.err.println("INFO: Matrices ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
