...
@Generated(value = "org.crossflow.java.Steam2Class", date = "2022-01-05T17:43:32.965002Z")
public class Input extends JobStream<Number> {
	...
	public Input(Workflow<MinimalWorkflowTasks> workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("CopierTask", (ActiveMQDestination) session.createQueue("InputPre.CopierTask." + workflow.getInstanceId()));
			destination.put("CopierTask", (ActiveMQDestination) session.createQueue("InputDestination.CopierTask." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createQueue("InputPost.CopierTask." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("CopierTask", postQ);			
					...
					try {
						...
						if (workflow.getCache() != null && workflow.getCache().hasCachedOutputs(job)) {
							...
							for (Job output : cachedOutputs) {
								if (output.getDestination().equals("Output")) {
									workflow.cancelTermination();
									((MinimalWorkflow) workflow).getOutput().send((Number) output, consumerId);
								}
	...
	public void addConsumer(InputConsumer consumer, String consumerId) throws Exception {
			...
			messageConsumer.setMessageListener(message -> {
				try {
					...
					if(messageText != null && messageText.length() > 0)
						number = workflow.getSerializer().deserialize(messageText);
					if(number != null)
						consumer.consumeInputWithNotifications(number);
					else
						System.err.println("INFO: Input ignoring null message.");
				} catch (Exception ex) {
					workflow.reportInternalException(ex);
				} finally { 
					try {
