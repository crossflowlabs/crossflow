...
@Generated(value = "org.crossflow.java.Workflow2Class", date = "2022-01-05T17:43:32.965002Z")
public class MinimalWorkflow extends Workflow<MinimalWorkflowTasks> {
	...
	protected ParallelTaskList<CopierTask> copierTasks = new ParallelTaskList<>();
	...
	public void run(long delay) throws Exception {
		...
		try {
			...
			if (isWorker()) {
				if (!tasksToExclude.contains(MinimalWorkflowTasks.COPIER_TASK)) {
						for(int i = 0; i <copierTasks.size(); i++){
							CopierTask task = copierTasks.get(i);
							input.addConsumer(task, "CopierTask");			
	...
	public CopierTask getCopierTask() {
		if(copierTasks.size()>0)
			return copierTasks.get(0);
		else 
			return null;
	}
	public ParallelTaskList<CopierTask> getCopierTasks() {
		return copierTasks;	
	}	
