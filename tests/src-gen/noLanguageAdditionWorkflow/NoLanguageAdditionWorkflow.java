...
@Generated(value = "org.crossflow.java.Workflow2Class", date = "2022-01-05T17:43:33.877651Z")
public class NoLanguageAdditionWorkflow extends Workflow<NoLanguageAdditionWorkflowTasks> {
	...
	protected Set<NoLanguageAdditionWorkflowTasks> tasksToExclude = EnumSet.noneOf(NoLanguageAdditionWorkflowTasks.class);

	public NoLanguageAdditionWorkflow() {
		this(Mode.MASTER, 1);
	}
	
	public NoLanguageAdditionWorkflow(Mode m) {
		this(m, 1);
	}
	
	public NoLanguageAdditionWorkflow(Mode mode, int parallelization) {
		super();
			
		this.parallelization = parallelization;	
			
		this.name = "NoLanguageAdditionWorkflow";
		this.mode = mode;
		...
		if (isWorker()) {
			if (!tasksToExclude.contains(NoLanguageAdditionWorkflowTasks.ADDER)) {
	...
	public void run(long delay) throws Exception {
		...
		try {
			...
			additions = new Additions(NoLanguageAdditionWorkflow.this, enablePrefetch);
			activeStreams.add(additions);
			additionResults = new AdditionResults(NoLanguageAdditionWorkflow.this, enablePrefetch);
	...
	public NoLanguageAdditionWorkflow createWorker() {
		NoLanguageAdditionWorkflow worker = new NoLanguageAdditionWorkflow(Mode.WORKER,parallelization);
		worker.setInstanceId(instanceId);
		return worker;
	}
	
	@Override
	public NoLanguageAdditionWorkflow excludeTask(NoLanguageAdditionWorkflowTasks task) {
	...
	@Override
	public NoLanguageAdditionWorkflow excludeTasks(Collection<NoLanguageAdditionWorkflowTasks> tasks) {
		for (NoLanguageAdditionWorkflowTasks t : tasks) {
			excludeTask(t);
		}
		return this;
	}
	...
	public static NoLanguageAdditionWorkflow run(String[] args) throws Exception {
		// Parse all values into an temporary object
		NoLanguageAdditionWorkflow argsHolder = new NoLanguageAdditionWorkflow();
		JCommander.newBuilder().addObject(argsHolder).build().parse(args);
		
		// Extract values to construct new object
		NoLanguageAdditionWorkflow app = new NoLanguageAdditionWorkflow(argsHolder.getMode(), argsHolder.getParallelization());
	...
	public static void main(String[] args) throws Exception {
		...
		if(argsHolder.isHelp()) {
			jct.setProgramName("NoLanguageAdditionWorkflow");
			jct.usage();
			System.exit(0);
			}
